...
//! These two decisions impact performance dramatically, but making
//! the optimal choices requires some attention. This crate employs
//! convenient abstraction boundaries between curves arithmetic,
...
//! curve over `F(q^2)` into some multiplicative group in a field,
...
//! ### Arithmetic
//!
//! An almost universally applicable optimization is to separate the
...
//! they hash, and which curve holds the signatures and public keys.
//! In other words, your desired aggregation techniques and usage 
//! characteristics should determine if you prefer the verification
//! equation `e(g1,sigma) = e(S,H(msg))` or the flipped form
//! `e(sigma,g2) = e(H(msg),S)`. See `UsualBLS` and `TinyBLS`.
//!
//! ### Aggregation
//!
//! We consider BLS signatures interesting because they support
...
//! the number of pairings, or size of the miller loop, by applying
...
//! delinearized optimizations, in which we multiply curve points
//! by values unforeseeable to the signers.
...
//! verification equation, beyond the naive information tracked
...
//! We thus encounter aggregation-like decisions that impact
...
//!
//! As a rule, we also attempt to batch normalize different arithmetic
//! outputs, but conceivably small signer set sizes might make this
//! a pessimization.
//!
//! 
//!
